...
import { createAdjustmentSchema } from '@schemas/payroll.Schema';
import {
...
export const CreateSingleAdjForm = ({ onClose, refetchData }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const result = await createAdjustment(formData).unwrap();
      const msg = result.message || 'Adjustment created successfully!';
      toast.success(msg);
      handleInternalClose();
      refetchData();
    } catch (error) {
      const message = getApiErrorMessage(error, 'Error creating adjustment.');
      toast.error(message);
    } finally {
      refetchData();
    }
