...
export default function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  const activePage = (itemId) => {
    ...
    // For all other routes, check if path starts with the item's route
    // This ensures nested routes (like /dashboard/onboarding/templates)
    // still highlight the parent menu item (onboarding)
    return currentPath.startsWith(`/dashboard/${itemId}`);
  };
  return (
    <>
