...
export const CreateEmployeeMultiStep = ({ refetchData }) => {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (
        formData.next_of_kin?.full_name &&
      ...
      toast.error(
        error?.data?.detail || error?.data?.error || 'An error occurred. Please try again.'
      );
    } finally {
        ...
        return (
          <>
            <div className="mb-6">
              ...
              <EmploymentDetailsTab register={register} errors={errors} setValue={setValue} />
              <PaymentMethodDetails register={register} errors={errors} setValue={setValue} />
            </div>
        ...
        <div className="relative z-50 animate-fadeIn" role="dialog" aria-modal="true">
          ...
          <div className="fixed inset-0 min-h-full flex items-center justify-center p-2 md:p-4 z-50">
            ...
            >
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto px-4 py-3">
                {renderTabContent()}
