...
const currentPageParam = parseInt(searchParams.get('page') || '1', 10);
  ...
  const {data: attendanceConsistencyData, isLoading:loadingConsistency,error:errorLoadingConsistency}=useGetAttendanceConsistencyQuery(filters, { refetchOnMountOrArgChange: true });
  ...
  // Calculate time breakdown percentages for visualization
  const calculateTimeBreakdown = () => {
    if (!attendanceData) return { working: 0, break: 0, overtime: 0 };
    
    const workingMinutes = (attendanceData.working_hours_formatted?.hours || 0) * 60 + 
...
const generateTimeMarkers = () => {
  ...
  const actualEndOffset =
    actualEndDate && expectedStartDate
      ...
      : actualStartOffset + ((attendanceData?.working_hours_formatted?.hours || 0) * 60 + (attendanceData?.working_hours_formatted?.minutes || 0)) / (totalRange / (1000 * 60)) * (100 / (totalRange / (1000 * 60 * 60)));
            ...
            ? [
                {
                  ...
                  value: attendanceData.clocked_in
                    ...
                    : "Not Clocked In",
                  subtext: `Arrival: ${attendanceData.arrival_status || "Unknown"}`,
                  icon: FiClock,
                ...
                },
                {
                  ...
                  value: `${attendanceData.working_hours_formatted.hours}h ${attendanceData.working_hours_formatted.minutes}m / ${attendanceData.expected_daily_hours} hrs`,
                  subtext: `Expected: ${attendanceData.expected_work_hours} hrs`,
                  icon: BsPersonCheck,
                  subtext: `Break: ${attendanceData.break_time_formatted.minutes}m`,
                ...
                },
                {
                  ...
                  value: `${attendanceData.monthly_total_hours || "0"} / ${attendanceData.expected_monthly_hours || "0"} hrs`,
                 subtext: "This Month",
                  icon: FiClock,
                ...
                {
                  title: "Overtime",
                  value: `${attendanceData.overtime_formatted.minutes}m`,
    ...
    {attendanceData?.clocked_in && (
      <div
        className={`absolute h-full rounded-md ${
          attendanceData?.arrival_status === "Late"
            ? "bg-red-400"
            : "bg-teal-500"
        }`}
        style={{
  ...
  <div className="relative flex justify-between text-xs text-gray-600 mt-2">
    {markers.map((mark, i) => (
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm flex justify-between items-center">
        ...
        <button className="px-4.5 
        py-2.5 bg-teal-600
         text-white rounded-lg flex items-center space-x-2 hover:bg-teal-700 transition-colors font-medium">
...
<div className="bg-white  rounded-xl shadow-sm">
  <div className="flex justify-between p-6 items-center mb-6 border-b pb-4">
    <h2 className="text-base font-semibold text-gray-800">
