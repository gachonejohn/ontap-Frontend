...
import { useGetOvertimeRequestsQuery, useUpdateOvertimeRequestMutation } from '@store/services/attendance/attendanceService';
...
const OvertimeRequests = () => {
  ...
  const { filters, currentPage, handleFilterChange, handlePageChange } = useFilters({
    initialFilters: {
      ...
      time_range: searchParams.get('time_range') || '1',
  ...
  const {
    ...
    error,
    refetch,
  } = useGetOvertimeRequestsQuery(queryParams, { refetchOnMountOrArgChange: true });

  const [updateOvertime, { isLoading: isUpdatingOvertime }] = useUpdateOvertimeRequestMutation();
  ...
  const handleApproveOvertime = async () => {
    if (!selectedItem) {
      toast.error('No overtime request selected');
      closeModal();
      return;
    }
    ...
    try {
      ...
      const msg = res?.message || 'Overtime approved successfully!';
      toast.success(msg);
      closeModal();
      await refetch();
    } catch (error) {
      ...
      if (error && typeof error === 'object' && 'data' in error && error.data) {
        ...
        toast.error(errorData.error || 'Error approving overtime!');
      } else {
  ...
  const handleRejectOvertime = async () => {
    ...
    try {
      ...
      const msg = res?.message || 'Overtime rejected successfully!';
      toast.success(msg);
      closeModal();
      await refetch();
    } catch (error) {
      ...
      if (error && typeof error === 'object' && 'data' in error && error.data) {
        const errorData = error.data;
        toast.error(errorData.error || 'Error rejecting overtime!');
      } else {
        toast.error('Unexpected error occurred. Please try again.');
      }
      closeModal();
  ...
  return (
    <div className="space-y-5 bg-white rounded-xl shadow-sm">
      ...
      <div className="flex p-6 flex-col md:flex-row md:items-center border-b md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-900">Overtime Requests</h1>
          <p className="text-sm text-gray-500 mt-1">
            All your Overtime activities, neatly organized and up to date
          </p>
        </div>
  ...
  <div className="flex flex-col">
    ...
    />
      ...
      {loadingData ? (
      ...
      ) : overtimeData && overtimeData.results.length > 0 ? (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
            {overtimeData.results.map((request) => {
              ...
              return (
                <div
                  key={request.id}
                  className="bg-white shadow-sm p-6 transition-shadow"
                >
                  ...
                  <div className="flex items-center space-x-4 mb-4 text-sm text-gray-600">
                    <div className="flex items-center space-x-2">
                      ...
                      <span>
                        {YearMonthCustomDate(request.date)}{' '}
          ...
          {/* Pagination */}
          {overtimeData.count > 0 && (
            <div className="bg-white rounded-xl shadow-sm p-4">
              <Pagination
                currentPage={currentPage}
                totalItems={overtimeData.count}
                pageSize={PAGE_SIZE}
        ...
        <div className="bg-white rounded-xl shadow-sm p-12">
          <NoDataFound message="No overtime requests found" />
        </div>
      )}
      ...
      <ActionModal
        ...
        onDelete={modalType === 'approve' ? handleApproveOvertime : handleRejectOvertime}
        isDeleting={isUpdatingOvertime}
