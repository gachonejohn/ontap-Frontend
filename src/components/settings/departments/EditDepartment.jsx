...
import { createDepartmentSchema } from '@schemas/companies/departmentPositionSchema';
...
import { useUpdateDepartmentMutation } from '@store/services/companies/departmentsService';
import { getApiErrorMessage } from '@utils/errorHandler';
export const EditDepartment = ({ refetchData, data }) => {
  const [isOpen, setIsOpen] = useState(false);

  const [updateDepartment, { isLoading: isCreating }] = useUpdateDepartmentMutation();
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const message = getApiErrorMessage(error, 'Error updating department.');
      toast.error(message);
    } finally {
      refetchData();
    }
