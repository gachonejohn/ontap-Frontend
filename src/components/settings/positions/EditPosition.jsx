...
import { useUpdatePositionMutation } from '@store/services/companies/positionsService';
import { getApiErrorMessage } from '@utils/errorHandler';
export const EditPosition = ({ refetchData, data }) => {
  const [isOpen, setIsOpen] = useState(false);

  const [updatePosition, { isLoading: isCreating }] = useUpdatePositionMutation();
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const message = getApiErrorMessage(error, 'Error updating position.');
      toast.error(message);
    } finally {
      refetchData();
    }
          ...
          <div className="fixed inset-0 min-h-full z-50 w-screen flex flex-col text-center md:items-center justify-center overflow-y-auto p-2 md:p-3 pointer-events-none">
            ...
            >
              <div className="sticky top-0 bg-white z-40 flex px-4 justify-between items-center py-4">
                <p className="text-sm md:text-lg lg:text-lg font-semibold">Edit position</p>
