...
export default function StaffOnboardingLayout({ children, isTemplatesView }) {
  const navigate = useNavigate();
  const location = useLocation();

  const { data, isLoading, error } = useGetOnboardingMetricsQuery(
    {},
    { refetchOnMountOrArgChange: true }
  );

  const isDetailView = /\/templates\/\d+/.test(location.pathname);
      ...
      <div className="flex justify-between items-center mb-3 py-2">
        <div className="flex flex-col gap-1.5">
          <div className="text-lg text-neutral-900 font-semibold">
            {isTemplatesView ? 'Onboarding Templates' : 'Staff Onboarding'}
          </div>
