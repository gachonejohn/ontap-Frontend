...
const LogTaskModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const initialAssignee = canViewAll ? "" : currentUser?.id;
      setFormData({
        title: "",
        description: "",
        status: "TO_DO",
        ...
        assignees: canViewAll ? [] : [currentUser?.id],
        department: "",
        start_date: "",
        due_date: "",
        progress_percentage: 0,
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        assignee: formData.assignee || formData.assignees?.[0] || currentUser.id,
        assignees: formData.assignees?.length > 0 ? formData.assignees : [formData.assignee || currentUser.id],
      };
